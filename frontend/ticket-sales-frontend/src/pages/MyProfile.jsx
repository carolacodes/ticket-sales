...
import { getMeRequest, updateMeRequest, updateRoleRequest, deleteMeRequest } from "@/api/user.api";
...
import { AlertTriangle, CheckCircle2, Copy, Shield, Trash2 } from "lucide-react";

// Si tenés endpoint de resend verificación, descomentá:
// import { resendVerifyEmailRequest } from "@/api/email.api";
...
function roleBadge(role) {
    if (role === "ORGANIZER") return "border-violet-500/25 bg-violet-600/10 text-violet-200";
...
export function MyProfile() {
    ...
    const displayUser = useMemo(() => me ?? ctxUser, [me, ctxUser]);
    ...
    async function switchRole(nextRole) {
        ...
        // importantísimo: tu backend devuelve accessToken nuevo
        // tu setAccessToken ya se ejecuta en axios interceptor? no.
        // así que lo hacemos acá (tenés libs/token.js)
    ...
    return (
        ...
        <div className="flex items-end justify-between gap-4">
            <div>
            <h1 className="text-5xl font-extrabold tracking-tight md:text-6xl">MY PROFILE</h1>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-[320px_1fr]">
            ...
            <CardContent className="p-6">
                ...
                <div className="grid h-28 w-28 place-items-center rounded-full bg-violet-600/20 ring-1 ring-violet-500/30">
                    <div className="text-3xl font-extrabold text-violet-200">
                    {initials(displayUser?.username || displayUser?.email || "")}
                    </div>
                </div>

                <div className="mt-4 text-xl font-semibold">{displayUser?.username || "—"}</div>
                <div className="mt-1 text-sm text-white/60">{displayUser?.email || "—"}</div>

                <Button className="mt-5 w-full rounded-2xl bg-violet-600 hover:bg-violet-500">
                    Upload Photo
                </Button>

                <div className="mt-4 grid w-full gap-2">
                    <div className={["flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm", verifiedBadge(!!displayUser?.emailVerified)].join(" ")}>
                    {displayUser?.emailVerified ? <CheckCircle2 className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
                    {displayUser?.emailVerified ? "Email Verified" : "Email Not Verified"}
                    </div>

                    <div className={["flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm", roleBadge(displayUser?.role)].join(" ")}>
            ...
            <Card className="border-white/10 bg-white/5">
                ...
                <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-5">
                    <div className="text-sm font-semibold">Switch to Organizer Mode</div>
                    <div className="mt-2 text-sm text-white/60">
                    Upgrade your account to create events, manage sales, and track ticket redemptions.
                    </div>

                    <div className="mt-4 flex items-center justify-between gap-3">
                    <div className="text-xs text-white/50">
                        Current: <span className="text-white/80">{displayUser?.role}</span>
                    </div>
                    ...
                    <div className="flex items-center rounded-2xl border border-white/10 bg-black/30 p-1">
                        ...
                        className={[
                            ...
                            displayUser?.role === "BUYER" ? "bg-violet-600 text-white" : "text-white/60 hover:text-white",
                        ].join(" ")}
                        ...
                        className={[
                            "px-4 py-2 text-sm rounded-xl transition",
                            displayUser?.role === "ORGANIZER" ? "bg-violet-600 text-white" : "text-white/60 hover:text-white",
                        ].join(" ")}
                    ...
                    {switching ? <div className="mt-3 text-xs text-white/50">Updating role…</div> : null}
                </div>
                </CardContent>
            </Card>
            ...
            <Card className="border-white/10 bg-white/5">
                ...
                <div className="mt-5 grid gap-4 md:grid-cols-2">
                    ...
                    <div className="text-[10px] uppercase tracking-widest text-white/45">Created At</div>
            ...
            <Card className="border-red-500/15 bg-red-500/5">
                ...
                <div className="flex items-center gap-3 text-red-200">
                    ...
                    <div className="text-sm text-red-200/60">
                        Once you delete your account, there is no going back. All your active tickets and purchase history will be permanently wiped.
                    </div>
                    </div>
                </div>
...
function DeleteAccountDialog({ open, onOpenChange, value, setValue, canDelete, onConfirm }) {
    return (
        ...
        <Dialog.Portal>
            ...
            <div className="px-6 pb-6 pt-5">
                ...
                Deleting your account will result in the immediate and irreversible loss of all purchased tickets, active memberships, and historical event data.
                You will no longer be able to access your digital wallet.
                </p>

                <div className="mt-6 text-[11px] uppercase tracking-widest text-white/40">
                To proceed, please type <span className="text-red-200">"DELETE"</span> below:
                </div>
